...
def _unittest_runner_command(pattern: str) -> list[str]:
    return [sys.executable, str((COMMANDS_DIR / "verify_bundle_runner.py").resolve()), pattern]
...
def main() -> int:
    ...
    python_files = sorted(_relative_bundle_path(path) for path in COMMANDS_DIR.glob("*.py"))
    python_files.extend(sorted(_relative_bundle_path(path) for path in SHARED_DIR.glob("*.py")))
    python_files.extend(sorted(_relative_bundle_path(path) for path in TOOLS_DIR.glob("*.py")))
    ...
    if args.include_smoke_matrix:
        steps.append(
            run_step(
                "smoke_matrix",
                [sys.executable, _relative_bundle_path(TOOLS_DIR / "run_smoke_matrix.py")],
            )
        )
