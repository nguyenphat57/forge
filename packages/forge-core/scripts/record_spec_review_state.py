...
VALID_DECISIONS = ("go", "revise", "blocked")


def build_report(args: argparse.Namespace) -> dict:
    if args.stage_status == "skipped" and not args.skip_reason:
        raise ValueError("Skipped spec-review state requires --skip-reason.")
    if args.stage_status != "skipped" and not args.activation_reason:
        raise ValueError("Non-skipped spec-review state requires --activation-reason.")
    if args.review_iteration > args.max_review_iterations:
...
def format_text(report: dict) -> str:
    lines = [
        "Forge Spec Review State",
        f"- Project: {report['project']}",
        f"- Profile: {report['profile']}",
        f"- Intent: {report['intent']}",
        f"- Stage status: {report['stage_status']}",
        f"- Decision: {report['decision']}",
        f"- Review iteration: {report['review_iteration']}/{report['max_review_iterations']}",
...
def persist_report(report: dict, output_dir: str | None) -> tuple[Path, Path]:
    artifact_dir = default_artifact_dir(output_dir, "spec-review") / slugify(report["project"])
    ...
    record_workflow_event("spec-review-state", report, output_dir=output_dir, source_path=json_path)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Record persisted spec-review stage state.")
    ...
    parser.add_argument("--decision", choices=VALID_DECISIONS, required=True, help="Spec-review decision")
    ...
    parser.add_argument("--summary", required=True, help="Stage summary")
    parser.add_argument("--review-iteration", type=int, default=1, help="Current review iteration")
    parser.add_argument("--max-review-iterations", type=int, default=3, help="Maximum review iterations")
    ...
    parser.add_argument("--persist", action="store_true", help="Persist spec-review state")
    ...
    try:
        ...
        if args.format == "json":
            ...
            print("\n".join(["Forge Spec Review State", "- Status: FAIL", f"- Error: {exc}"]))
        return 1
