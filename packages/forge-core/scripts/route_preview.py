...
def prompt_structure_score(text: str, phrases: list[str]) -> int:
    return sum(
        ...
        if keyword_in_text(normalize_text(phrase), text)
    )
...
def looks_like_direction_selection_request(prompt_text: str) -> bool:
    ...
    choice_score = prompt_structure_score(
        normalized,
        [
            "hay",
            "hoac",
...
def build_delegation_packet_blueprints(strategy_key: str, execution_pipeline_key: str | None, registry: dict) -> list[dict]:
