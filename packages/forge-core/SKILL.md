...
**Minimal routing policy:** For `REVIEW`, `SESSION`, and task `small`, Forge prioritizes prompt-led routing. Repo signals at this time will not automatically pull additional local companions or escalation profiles if the prompt does not clearly state the need.
