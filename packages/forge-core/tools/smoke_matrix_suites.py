...
def _resolve_skill_command_path(skill_source_name: str, package_name: str, command_name: str) -> Path:
    candidates = (
...
def run_router_suite() -> list[dict]:
    router_script = ROOT_DIR / "tools" / "check_workspace_router.py"
...
def run_response_contract_suite() -> list[dict]:
    contract_script = ROOT_DIR / "tools" / "check_response_contract.py"
    return _run_json_suite(
        "response-contract",
        RESPONSE_CONTRACT_CASES,
        command_builder=lambda case: (
