...
Date: 2026-04-23
Status: `3.2.1` is the current stable release after the stable-line metadata refresh, the delegation-setting cleanup, the stronger subagent auto-activation gate, the Visual Companion ownership move, and the latest verification gate.
...
- release-facing docs now align on `3.2.1` as the stable source version

## Verdict

Forge `3.2.1` is in a reasonable state for public stable release under the current policy.
