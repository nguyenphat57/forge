...
Date: 2026-04-13
Status: `2.5.0` is the current stable release after the maintenance-only closure sync and historical-roadmap retirement cut.
...
- `python scripts/verify_repo.py --profile fast` passed on 2026-04-13
...
- release-facing docs now align on `2.5.0` as the stable source version

## Verdict

Forge `2.5.0` is in a reasonable state for public stable release under the current policy.
