...
Date: 2026-04-18
Status: `2.8.0` is the current stable release after the installed-workflow shortcut materialization slice and the latest canonical verification gate.
...
- `python scripts/verify_repo.py` passed on 2026-04-18
...
- release-facing docs now align on `2.8.0` as the stable source version

## Verdict

Forge `2.8.0` is in a reasonable state for public stable release under the current policy.
