...
Date: 2026-04-21
Status: `2.13.0` is the current stable release after the wave-execution engine release sync and the latest verification gate.
...
- release-facing docs now align on `2.13.0` as the stable source version

## Verdict

Forge `2.13.0` is in a reasonable state for public stable release under the current policy.
