...
Date: 2026-04-23
Status: `3.2.2` is the current stable release after the Windows Forge Codex UTF-8 activation fix, the installed-bundle Vietnamese preference repro coverage, and the latest verification gate.
...
- release-facing docs now align on `3.2.2` as the stable source version

## Verdict

Forge `3.2.2` is in a reasonable state for public stable release under the current policy.
