...
def write_build_manifest(destination: Path, package_name: str, host: str, source: str) -> None:
    manifest = {
...
def build_core_bundle() -> dict:
    destination = DIST_DIR / "forge-core"
    clean_dir(destination)
    copy_tree(CORE_DIR, destination)
    write_build_manifest(destination, "forge-core", "generic", "packages/forge-core")
    return {"name": "forge-core", "path": str(destination), "host": "generic"}


def build_adapter_bundle(package_dir: Path) -> dict:
    ...
    write_build_manifest(destination, manifest["name"], manifest["host"], str(package_dir.relative_to(ROOT_DIR)))
    return {"name": manifest["name"], "path": str(destination), "host": manifest["host"]}


def build_all() -> list[dict]:
    DIST_DIR.mkdir(parents=True, exist_ok=True)
    outputs = [build_core_bundle()]
    for package_dir in sorted(PACKAGES_DIR.iterdir()):
        ...
        if not (package_dir / "adapter.json").exists():
            continue
        outputs.append(build_adapter_bundle(package_dir))
    return outputs


def format_text(outputs: list[dict]) -> str:
    lines = ["Forge Release Build"]
    for item in outputs:
